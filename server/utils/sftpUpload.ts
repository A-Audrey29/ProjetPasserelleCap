...
export async function uploadToSFTP(
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`📁 [SFTP] Mode développement - Skip upload SFTP pour: ${path.basename(localFilePath)}`);
    return true; // Considéré comme succès en dev
  }

  // Vérifier que le mot de passe SFTP est configuré
  if (!process.env.SFTP_PASSWORD) {
    console.error('❌ [SFTP] SFTP_PASSWORD non configuré dans les variables d\'environnement');
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      console.error('⚠️ [SFTP] Erreur lors de la fermeture de la connexion:', closeError);
    }
  }
}
...
export async function testSFTPConnection(): Promise<boolean> {
  ...
  try {
    ...
    console.log(`   ✅ Dossier navettes: ${navettesExists ? 'OK' : '❌ INTROUVABLE'}`);
    console.log(`   ✅ Dossier bilans: ${bilansExists ? 'OK' : '❌ INTROUVABLE'}`);
