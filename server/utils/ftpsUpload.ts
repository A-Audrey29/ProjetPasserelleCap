...
function getFTPSConfig(): FTPSConfig {
  ...
  if (!password) {
    throw new Error(
      "❌ Aucun mot de passe FTP trouvé (FTP_PASS ou FTP_PASSWORD manquant dans les variables d'environnement)",
    );
  }
  ...
  return {
    ...
    password, // ✅ utilise la variable combinée
...
class FTPSClient {
  ...
  async connect(maxRetries: number = 3): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `🔌 [FTPS] Tentative ${attempt}/${maxRetries} - Connexion à ${this.config.host}:${this.config.port}...`,
        );
        console.log(`👤 [FTPS] Utilisateur: ${this.config.user}`);
        console.log(
          `🔒 [FTPS] Mode sécurisé: ${this.config.secure ? "OUI (TLS)" : "NON"}`,
        );
        ...
        await this.client.access({
          ...
          secureOptions: {
            ...
            rejectUnauthorized:
              process.env.FTP_HOST === "millet.o2switch.net" ? true : false,
        ...
        lastError = error;
        console.error(
          `❌ [FTPS] Échec tentative ${attempt}/${maxRetries}: ${error.message}`,
        );
    ...
    throw new Error(
      `Échec de connexion FTPS après ${maxRetries} tentatives: ${lastError?.message}`,
    );
  }
  ...
  async uploadFile(
    ...
    try {
      console.log(
        `📁 [FTPS] Vérification du répertoire distant: ${remoteDirectory}`,
      );
      await this.client.ensureDir(remoteDirectory);
      console.log(`✅ [FTPS] Répertoire distant prêt: ${remoteDirectory}`);
    } catch (error: any) {
      console.error(
        `⚠️ [FTPS] Impossible de créer le répertoire ${remoteDirectory}: ${error.message}`,
      );
...
export async function uploadToFTPS(
  ...
  if (process.env.NODE_ENV !== "production") {
    console.log(
      `📁 [FTPS] Mode développement - Upload FTPS ignoré pour: ${path.basename(localFilePath)}`,
    );
    return true;
  }

  console.log(
    `🚀 [FTPS] Début de l'upload ${uploadType} → ${path.basename(localFilePath)}`,
  );

  // Vérifier la configuration
  if (!process.env.FTP_PASSWORD) {
    console.error(
      "❌ [FTPS] FTP_PASSWORD non définie dans les variables d'environnement",
    );
    console.error(
      "💡 [FTPS] Configurez FTP_PASSWORD sur Render pour activer l'upload FTPS",
    );
    return false;
  }
  ...
  try {
    ...
    console.error(
      `   - Serveur: ${process.env.FTP_HOST || "millet.o2switch.net"}`,
    );
    console.error(`   - Port: ${process.env.FTP_PORT || "21"}`);
    console.error(`   - Utilisateur: ${process.env.FTP_USER || "kalo4499"}`);
    console.error(
      `   - Mot de passe configuré: ${process.env.FTP_PASSWORD ? "OUI" : "NON"}`,
    );
