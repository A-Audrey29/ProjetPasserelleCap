...
/*
CREATE TYPE "public"."email_status" AS ENUM('intercepted', 'sent', 'viewed', 'archived', 'error');--> statement-breakpoint
CREATE TYPE "public"."fiche_state" AS ENUM('DRAFT', 'SUBMITTED_TO_FEVES', 'ASSIGNED_EVS', 'ACCEPTED_EVS', 'EVS_REJECTED', 'CONTRACT_SIGNED', 'ACTIVITY_DONE', 'FIELD_CHECK_SCHEDULED', 'FIELD_CHECK_DONE', 'FINAL_REPORT_RECEIVED', 'CLOSED', 'ARCHIVED');--> statement-breakpoint
...
CREATE TABLE "email_logs" (
	...
	"html" text NOT NULL,
	"meta" json,
	"status" "email_status" DEFAULT 'intercepted' NOT NULL,
	"error" text,
	"message_id" text,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"viewed_at" timestamp,
...
CREATE TABLE "workshop_enrollments" (
	...
	"report_uploaded_by" varchar,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "unique_fiche_workshop" UNIQUE("fiche_id","workshop_id")
);
--> statement-breakpoint
...
CREATE INDEX "workshops_objective_idx" ON "workshops" USING btree ("objective_id" text_ops);--> statement-breakpoint
CREATE INDEX "email_logs_created_at_idx" ON "email_logs" USING btree ("created_at" timestamp_ops);--> statement-breakpoint
CREATE INDEX "email_logs_status_idx" ON "email_logs" USING btree ("status" enum_ops);--> statement-breakpoint
CREATE INDEX "migrations_executed_at_idx" ON "migrations" USING btree ("executed_at" timestamp_ops);--> statement-breakpoint
CREATE INDEX "migrations_name_idx" ON "migrations" USING btree ("name" text_ops);--> statement-breakpoint
CREATE INDEX "workshop_enrollments_evs_idx" ON "workshop_enrollments" USING btree ("evs_id" text_ops);--> statement-breakpoint
CREATE INDEX "workshop_enrollments_fiche_idx" ON "workshop_enrollments" USING btree ("fiche_id" text_ops);--> statement-breakpoint
CREATE INDEX "workshop_enrollments_session_idx" ON "workshop_enrollments" USING btree ("workshop_id" int4_ops,"evs_id" int4_ops,"session_number" int4_ops);--> statement-breakpoint
CREATE INDEX "workshop_enrollments_workshop_idx" ON "workshop_enrollments" USING btree ("workshop_id" text_ops);--> statement-breakpoint
...
CREATE INDEX "fiche_navettes_state_idx" ON "fiche_navettes" USING btree ("state" enum_ops);
*/